...
| Generated beta / preview | Recursive Observe's generic Agent Skill and Claude package are generated beta; its Codex package is generated preview | Generated artifacts, source receipt, standalone execution, zero-write coexistence, and an isolated Claude Code install pass; live Codex installation and hosted persistence remain acceptance work |
| Generated preview | Recursive Guard's separate Codex package | Installation is inert; activation requires a reviewed repository policy and a separate hook-trust decision. The package is receipt-bound and fixture-tested, but is a guardrail rather than a sandbox |
