...
| Generated beta | Recursive Observe's generic Agent Skill, Claude package, and local Codex package | Generated artifacts, raw-byte source receipt, standalone execution, zero-write coexistence, historical version 1 Claude acceptance, and Windows Codex 0.145.0 version 2 immutable-ref install/cache/rollback pass; fresh Claude version 2 installation and hosted persistence remain acceptance work |
| Generated beta | Recursive Guard's separate local Codex package | Installation is inert; activation requires a reviewed repository policy and a separate hook-trust decision. The package is receipt-bound, fixture-tested, and consumer-installed, but is a guardrail rather than a sandbox |
| Generated beta | Namespaced Learn, Verify, and Coordinate provider packages | Receipt-bound generic, Claude, and Codex artifacts plus isolated consumer evidence exist; hosted use and Coordinate remote services remain unverified or unshipped |
| Generated experimental | Namespaced Lab provider package | Preview-only Brainstorm and Roadmap artifacts, hash receipts, isolated install/uninstall, and zero-write consumer evidence exist; model quality, automatic selection, connectors, and hosted use remain unverified or unshipped |
