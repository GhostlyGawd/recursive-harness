...
| Portable multi-agent distribution | Observe, Learn, Verify, Coordinate, Guard, and experimental Lab have receipt-bound local consumer proof; v0.1.2 is released and the combined advisory suite passes a real Codex preflight; the full hook harness remains Claude-specific and public-directory review is pending | In progress | Preserve the provider contract, keep Coordinate local-only, keep Lab preview-only, and finish official marketplace review plus fresh public-install evidence |
