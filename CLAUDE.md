...
## Environment facts — fresh/cloud clones
...
- Fresh clones can omit release tags: run `git fetch origin --tags` before the release/
  plugin tests or `scripts/build_public_plugin.py`. The preflights added in PR #272 print
  these same fixes, but only after a failure.
