...
def header(g, overlay, stamp):
    by_t = _by_type(g)
    by_e = _by_etype(g)
    p = overlay["predictions"]
    L = []
    L.append("# The Harness Atlas")
    L.append("")
    L.append("> **A holistic, machine-truth map of the recursive-harness - every component, how "
             "they flow and synergize, and where the system strains.**")
    L.append(">")
    ...
    L.append(f"| Prediction calibration | **{hr}** hit-rate ({p['hit']} hit / {p['miss']} miss / "
             f"{p['unscored']} open over {p['total']}) |")
    ...
    L.append("**How to read this document.** Each section is a different *lens* on the one graph. "
             "Diagrams render natively on GitHub (Mermaid). The structural views change only when the "
             "harness changes; the **Observability snapshot** at the end is a point-in-time read of "
             "the live `state/` ledgers (the canonical main-checkout copy).")
    L.append("")
    L.append("---")
    return "\n".join(L)
...
def section_snapshot(overlay, heal, clusters):
    L = []
    L.append("## 9. Observability snapshot  `[point-in-time read of state/]`")
    L.append("")
    L.append("Live machine-state, not topology. This is the section that drifts session to session "
             "- regenerate to re-sync. It answers *where the system strains*.")
    L.append("")
    # bottlenecks: prediction hit-rate by category
    p = overlay["predictions"]
    ...
    L.append("---")
    L.append("")
    L.append("_Atlas generated by `cartograph/atlas.py`. To keep this map true as the harness "
             "evolves, regenerate after structural changes (or wire it into `/retro` / CI). The "
             "cartograph gate (`extract.py --check`) already fails CI on un-baselined structural "
             "rot._")
    return "\n".join(L)
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Generate the Harness Atlas (cartograph/ATLAS.md).")
    ap.add_argument("out", nargs="?", default=None,
                    help="output path; '-' for stdout; default cartograph/ATLAS.md")
    ap.add_argument("--out", dest="out_flag", default=None, help="output path (alias)")
    args = ap.parse_args(argv)
    target = args.out_flag or args.out
    text = build_atlas()
    if target == "-":
        sys.stdout.write(text)
        return 0
    if not target:
        target = os.path.join(ROOT, "cartograph", "ATLAS.md")
    with open(target, "w", encoding="utf-8", newline="\n") as fh:
        fh.write(text)
    sys.stdout.write(f"atlas: wrote {extract.rel(target)} "
