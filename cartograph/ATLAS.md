...
| Graph | Value |
|---|---|
| Nodes | **152** (adr=12, agent=4, cli=16, command=14, config=3, evals=10, event=6, hook=21, kernel=1, lint=1, session=35, skill=23, state=6) |
| Edges | **322** (born_in=60, cites=47, fires_on=22, invokes=43, nudges=68, references=34, spawns=18, touches=12, wires=18) |
...
| **Self-improvement loops** | 57 | inner predict->act->score / middle /retro / outer /meta-retro - the three nested learning cycles. |
...
| **Observability - self-awareness** | 41 | cartograph (this map) + mission_control TUI + the state/ hot ledgers: the harness watching itself. |
...
|---|---|---:|---:|
| `/retro` | command | 15 | 47 |
| `/harness-pr` | command | 14 | 47 |
| `0008-feature-flags-config` | adr | 13 | 14 |
| `/meta-retro` | command | 11 | 47 |
| `0004-dual-config-topology` | adr | 10 | 50 |
| `harness-authoring` | skill | 9 | 47 |
| `/run-evals` | command | 8 | 47 |
| `critic` | agent | 7 | 49 |
| `harness-auditor` | agent | 7 | 48 |
| `harness outcome` | cli | 7 | 48 |
| `harness predict` | cli | 7 | 48 |
| `PreToolUse` | event | 7 | 8 |
| `routing-learnings` | skill | 6 | 47 |
| `harness followup` | cli | 6 | 48 |
| `harness stats` | cli | 6 | 48 |
...
|---|---|---:|---|
| lineage | session | 35 | provenance ancestry (sessions) |
...
| transporter | cli | 16 | move metabolites in/out of the ledgers (CLI) |
...
| `born_in` | 60 | artifact → session (provenance lineage) |
| `cites` | 47 | artifact → skill (skill references) |
| `invokes` | 43 | artifact → CLI subcommand (harness <cmd>) |
...
|---|---|---:|---|---|
| **cartograph** | observability | 27 | shipped | Read-only machine-truth extractor: graph + gate + audit + oracle + diff + html + this atlas. |
...
| **fleet** | observability | 37 | shipped | Append-only, self-reaping typed event log (Agent Mail) for cross-session/worktree coordination. |
| **hooks** | enforcement | 43 | shipped | 17 lifecycle hooks: the write-lock guard, worktree/trunk concurrency guards, gates, loggers. |
| **evals** | selection | 54 | shipped | In-session regression corpus (ADR-0003, no headless); replayed via /run-evals, graded by check.py. |
...
| **memory** | kernel | 18 | shipped | Versioned cold knowledge: ADRs, user-model (evidence-tagged), calibration rollups, heal ledgers. |
