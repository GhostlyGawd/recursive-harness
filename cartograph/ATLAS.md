...
| Graph | Value |
|---|---|
| Nodes | **147** (adr=12, agent=4, cli=14, command=14, config=3, evals=10, event=6, hook=21, kernel=1, lint=1, session=34, skill=21, state=6) |
| Edges | **314** (born_in=59, cites=45, fires_on=22, invokes=40, nudges=66, references=34, spawns=18, touches=12, wires=18) |
...
| **Observability - self-awareness** | 40 | cartograph (this map) + mission_control TUI + the state/ hot ledgers: the harness watching itself. |
...
| `0004-dual-config-topology` | adr | 10 | 49 |
...
|---|---|---:|---|
| lineage | session | 34 | provenance ancestry (sessions) |
...
| `born_in` | 59 | artifact → session (provenance lineage) |
| `cites` | 45 | artifact → skill (skill references) |
| `invokes` | 40 | artifact → CLI subcommand (harness <cmd>) |
...
| **memory** | kernel | 17 | shipped | Versioned cold knowledge: ADRs, user-model (evidence-tagged), calibration rollups, heal ledgers. |
