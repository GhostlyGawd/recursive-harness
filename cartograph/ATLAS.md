...
**Build stamp** - generated `2026-08-15` from extract.py @ `5a2bae4`.

| Graph | Value |
|---|---|
| Nodes | **167** (adr=13, agent=4, cli=21, command=14, config=3, evals=12, event=6, hook=21, kernel=1, lint=1, session=38, skill=27, state=6) |
| Edges | **334** (born_in=65, cites=51, fires_on=22, invokes=46, nudges=66, references=34, spawns=19, touches=13, wires=18) |
...
|---|---:|---|
| **Kernel - the genome** | 17 | CLAUDE.md prime directives + ADRs + regulatory config; the conserved DNA every session reads. |
| **Self-improvement loops** | 66 | inner predict->act->score / middle /retro / outer /meta-retro - the three nested learning cycles. |
...
flowchart LR
  ...
  n_hook_session_end -->|"writes"| n_state_candidates
  n_state_candidates -->|"reads"| n_hook_heal_autocapture
  n_state_candidates -->|"reads"| n_hook_session_end
  ...
  n_state_sessions -->|"reads"| n_hook_session_end
  ...
  n_state_skill_usage -->|"reads"| n_hook_log_skill_use
...
| **candidates.jsonl** | `heal_autocapture.py`, `session_end.py` | `heal_autocapture.py`, `session_end.py` |
| **corrections.jsonl** | - | `log_correction.py`, `session_end.py`, `stop_retro_gate.py` |
| **predictions.jsonl** | `session_start.py` | `session_end.py`, `session_start.py` |
| **sessions.jsonl** | `session_end.py`, `session_start.py` | `session_end.py`, `session_start.py`, `stop_cadence_gate.py` |
| **skill_usage.jsonl** | `log_skill_use.py` | `log_skill_use.py`, `session_end.py` |
...
|---|---|---:|---:|
| `/harness-pr` | command | 14 | 49 |
| `/retro` | command | 14 | 49 |
| `0008-feature-flags-config` | adr | 13 | 14 |
| `/meta-retro` | command | 11 | 49 |
| `harness-authoring` | skill | 10 | 49 |
| `0004-dual-config-topology` | adr | 10 | 52 |
| `/run-evals` | command | 8 | 49 |
| `critic` | agent | 8 | 52 |
| `routing-learnings` | skill | 7 | 49 |
| `harness-auditor` | agent | 7 | 50 |
| `harness outcome` | cli | 7 | 50 |
| `harness predict` | cli | 7 | 50 |
| `PreToolUse` | event | 7 | 8 |
| `auto-healer` | skill | 6 | 49 |
| `harness followup` | cli | 6 | 50 |
...
| ribosome | skill | 27 | translate trigger-signals into procedure (skills) |
| enzyme | hook | 21 | catalyse reactions at lifecycle membranes (hooks) |
| transporter | cli | 21 | move metabolites in/out of the ledgers (CLI) |
| receptor | command | 14 | user-initiated pathway entry points (commands) |
| nucleus | adr, kernel | 14 | versioned cold knowledge (genome / conserved genes) |
...
|---|---:|---|
| `nudges` | 66 | artifact → command (/cmd pointer) |
| `born_in` | 65 | artifact → session (provenance lineage) |
| `cites` | 51 | artifact → skill (skill references) |
| `invokes` | 46 | artifact → CLI subcommand (harness <cmd>) |
...
| **fleet** | observability | 43 | shipped | Append-only, self-reaping typed event log (Agent Mail) for cross-session/worktree coordination. |
| **hooks** | enforcement | 64 | shipped | 17 lifecycle hooks: the write-lock guard, worktree/trunk concurrency guards, gates, loggers. |
| **evals** | selection | 41 | shipped | In-session regression corpus (ADR-0003, no headless); replayed via /run-evals, graded by check.py. |
| **tests** | selection | 107 | shipped | Harness-level pytest integration suite for guards, features, hooks, state - run in CI. |
| **lint** | selection | 2 | shipped | Self-governance linter: budgets, falsifiable claims, provenance, autonomy firewall. |
| **memory** | kernel | 21 | shipped | Versioned cold knowledge: ADRs, user-model (evidence-tagged), calibration rollups, heal ledgers. |
