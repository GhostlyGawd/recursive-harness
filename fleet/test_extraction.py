...
The per-module `test_*_imports_stdlib_only` tests assert each file imports only stdlib + the
engine. This goes further: it copies every fleet/*.py into a fresh temp package with the harness
root NOT on sys.path, and runs the engine/view/CLI suites there in a subprocess. If they pass,
the package has zero harness coupling and Phase-5 extraction is a non-event (ARCHITECTURE
decision #5 / proposals/2026-06-22-agent-mail-product.md §0).
